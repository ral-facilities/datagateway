...
describe('Datafile preview', () => {
  ...
  describe('Facility cycle hierarchy', () => {
    beforeEach(() => {
      cy.visit(
  ...
  describe('Data publication hierarchy', () => {
    beforeEach(() => {
      cy.visit(
        '/browseDataPublications/instrument/13/dataPublication/57/investigation/46/dataset/15/datafile/1085'
      );
    });

    it('should load datafile previewer in data publication hierarchy', () => {
      // should be able to see the content of the text file
      cy.contains('First line').should('exist');
      cy.contains('Second line').should('exist');
      cy.contains('Third line').should('exist');
